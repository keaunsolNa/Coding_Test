...
**Difficulty:** Medium | **Tags:** Two Pointers, String, Binary Search, Rolling Hash, String Matching, Hash Function
...
- `s[i..(i + a.length - 1)] == a`
- There exists an index `j` such that: - `0 <= j <= s.length - b.length`
- `s[j..(j + b.length - 1)] == b`
- `|j - i| <= k`
