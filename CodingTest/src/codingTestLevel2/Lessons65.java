...
public class Lessons65 {
	...
	List<Character> optKind = new ArrayList<>();
	Long MAX = (long) 0;
	...
	public static void main(String[] args) {
		...
		long answer = lessons65.solution(s);
...
//		참가자에게 주어진 연산 수식이 담긴 문자열 expression이 매개변수로 주어질 때, 우승 시 받을 수 있는 가장 큰 상금 금액을 return 하도록 solution 함수를 완성해주세요.
    ...
    public Long calcOpt(long a, long b, char opt) {
        long res = 0;
        if (opt == '+') {
            res = a + b;
        } else if (opt == '-') {
    ...
    public Long calc(List<Character> order) {
        ...
        System.out.println("After clac : " + Math.abs(copyNumList.get(0)));
        return Math.abs(copyNumList.get(0));
    }
    
    public void priorityOrder(List<Character> order) {
