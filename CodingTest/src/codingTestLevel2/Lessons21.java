...
public class Lessons21 {
	...
	public static void main(String[] args) {
		...
		int[] answer = lessons21.solution(n, words);
...
//		가장 먼저 탈락하는 사람의 번호와 그 사람이 자신의 몇 번째 차례에 탈락하는지를 구해서 return 하도록 solution 함수를 완성해주세요.
		...
		for (int i = 1; i < words.length; i++) {
			...
			if(word.charAt(word.length() - 1) != words[i].charAt(0)) {
				...
				System.out.println("틀린 차례 : " + Math.ceil((i+2)/n));
				answer[0] = people;
				answer[1] = (int)Math.ceil((double)(i+1)/n);
				return answer;
			} else {
				...
				if(people <= n) {
					...
					if(strMap.size() < i + 1) {
						System.out.println("중복 키 발생");
						System.out.println("틀린 사람 : " + people);
						System.out.println("loof : " + i);
						System.out.println("틀린 차례 : " + Math.ceil((i+1)/n));
						answer[0] = people;
						answer[1] = (int)Math.ceil((double)(i+1)/n);
						return answer;
					}
					...
					if(strMap.size() < i + 1) {
						...
						System.out.println("틀린 사람 : " + people);
						System.out.println("loof : " + i);
						System.out.println(Math.ceil((double)(i+1)/n));
						System.out.println("틀린 차례 : " + Math.ceil((i+1)/n));
						answer[0] = people;
						answer[1] = (int)Math.ceil((double)(i+1)/n);
						return answer;
