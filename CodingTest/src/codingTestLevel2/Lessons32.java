...
public class Lessons32 {
	public static void main(String[] args) {
		...
		int answer = lessons32.solution("FRANCE", "french");
...
//		두 문자열 사이의 자카드 유사도 J("FRANCE", "FRENCH") = 2/8 = 0.25가 된다.
        ...
        int result = union(str1, str2);
        
        return result;
        
    }
	
	public int union(String str1, String str2){
		...
		for (int i = 0; i < charArr.length -1; i++) {
			String temp = "" + charArr[i] + charArr[i+1];
			if(Pattern.matches(match, temp)) {
				strSet1.add("" + charArr[i] + charArr[i+1]);
			}
		}
		
		System.out.println("str1 원소 : " + strSet1);
		
		for (int i = 0; i < charArr2.length -1; i++) {
			String temp = "" + charArr2[i] + charArr2[i+1];
			if(Pattern.matches(match, temp)) {
				strSet2.add("" + charArr2[i] + charArr2[i+1]);
			}
		}
		
		System.out.println("str2 원소 : " + strSet2);
		...
		unionList.addAll(strSet2);
		
		double intersectionList_length = intersectionList.size();
		double unionList_length = unionList.size();
		
		if(unionList_length == 0) {
			return 65536;
		} 
			
		return (int)(intersectionList_length/unionList_length* 65536);
