...
public class Lessons57 {
	...
	public static void main(String[] args) {
		...
		Lessons57 lessons57 = new Lessons57();
		int answer = lessons57.solution(437674, 3);
...
//		정수 n과 k가 매개변수로 주어집니다. n을 k진수로 바꿨을 때, 변환된 수 안에서 찾을 수 있는 위 조건에 맞는 소수의 개수를 return 하도록 solution 함수를 완성해 주세요.
        ...
        String baseRadix = tobaseRadix(n, k);
        System.out.println(baseRadix);

        String[] strArr = baseRadix.split("0");
        for (String string : strArr) {
        	if(!string.equals("")) {
	        	if(!(Long.parseLong(string) < 2)) {
	        		if(isPrime(Long.parseLong(string))){
	        			primeList.add(Long.parseLong(string));
	        		}
	        	}
        	}
		}
